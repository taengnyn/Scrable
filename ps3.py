...
#
# Name          : Bilovol Dariia, KM-24
# Collaborators : <your collaborators>
# Time spent    : days and nights 
...
def get_word_score(word, n):
    ...
    if word in word_list:
        ...
        component1 = 0
        for letter in word:
            component1 += SCRABBLE_LETTER_VALUES.get(letter) 
        word_length = len(word)
        comp = (7 * word_length - 3  * (n - word_length))
        component2 = max(1, comp)
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    display_hand(hand)
    replace = input('Would you like to substitute a letter?')
    # If user answered yes
    if replace.lower() == 'yes':
        repl = input('Which letter would you like to replace: ')
        hand = substitute_hand(hand, repl)
    elif replace.lower() == 'no': 
        while calculate_handlen(hand) > 1:
            print('Current Hand: ', end = '')
            display_hand(hand)  
            word = input('Enter word or "!!" to finish the game: ')
            is_valid_word(word, hand, word_list)
            if word == '!!':
                print('Total score for hand: ', score)
                print('_'*20)
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    hand =  deal_hand(HAND_SIZE)
    n = calculate_handlen(hand)
    hand_t = int(input('Enter total number of hands: '))
    while t < hand_t:
        hand_copy = hand.copy()
        if hand_t == 1:
            ...
            if replace.lower() == 'yes':
                ...
                #hand_tot = play_hand(hand, word_list)
                print('Current Hand: ', end = '')
                display_hand(hand)  
                word = input('Enter word or "!!" to finish the game: ')
                is_valid_word(word, hand, word_list)
                if word == '!!':
                    ...
                    if calculate_handlen(hand) <= 1:
                        ...
                        if to_replay.lower() == 'yes':
                            ...
                            hand =  deal_hand(HAND_SIZE)
        continue
    print('Total score over all hands:', all_total)
